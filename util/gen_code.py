...
class HTMLTableReader(HTMLParser):
    ...
    def get_table(self, content: str) -> list[tuple[str, str]]:
        self.feed(content)
        return [
            tuple(self.table_data[i:i + 2])
            for i in range(0, len(self.table_data), 2)
        ]
...
def write_footer(fh: t.TextIO, fc: t.TextIO) -> None:
    fh.write(("#endif /* SATCAT_CODE_H */\n"))


def str5_to_code(s: str) -> int:
    return ord(s[0]) + (ord(s[1]) << 8) + (ord(s[2]) << 16) + (
        ord(s[3]) << 24) + (ord(s[4]) << 32)


def write_body_5byte(fh: t.TextIO, fc: t.TextIO, table: list[tuple[str, str]],
                     prefix: str, name: str, fname: str) -> str:
    bodyh = "".join([
        f'#define {prefix}_{r[0].ljust(5)} (sc_code_t){str5_to_code(r[0].ljust(5))}UL\n'
        for r in table
    ])
    fh.write((f"/* {name} */\n"
              f"{bodyh}"
    ...
    funcc = "".join(
        [f'\tcase {prefix}_{r[0]}:\n\t\treturn "{r[1]}";\n' for r in table])
    fc.write((f"const char *sc_{fname}_str(const sc_code_t code)\n"
              "{\n"
              "\tswitch (code) {\n"
              f"{funcc}"
              "\tdefault:\n"
...
def write_status_enum(fh: t.TextIO, fc: t.TextIO) -> None:
    fh.write(("/* Status */\n"
              ...
              "\n"))
    fc.write(("const char *sc_status_str(const char code)\n"
              "{\n"
              "\tswitch(code) {\n"
...
def main() -> None:
    tabs = (
        {
            "url": "http://www.celestrak.org/satcat/sources.php",
            "name": "Source",
            "prefix": "SCSRC",
            "fname": "source"
        },
        {
            "url": "http://www.celestrak.org/satcat/launchsites.php",
            "name": "Launch Site",
            "prefix": "SCSITE",
            "fname": "launch_site"
        },
